...
export default function ImportarManifiestoPage() {
  ...
  const selectFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setArchivo(file); setPreview(null); setProgress(null); setError(""); setQuery(""); setAlertsOnly(false);
  ...
  const loadPreview = async (file: File) => {
    ...
    try {
      ...
      setPreview(data);
    } catch (e) { setError(e instanceof Error ? e.message : "No fue posible analizar el manifiesto."); }
  ...
  const startImport = async () => {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "No fue posible iniciar la importación."); }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      ...
      } catch { /* polling is best effort; the last real state remains visible */ }
    }, 900);
    return () => window.clearInterval(timer);
  }, [progress?.jobId, progress?.status]);

  const clear = () => { setArchivo(null); setPreview(null); setProgress(null); setError(""); setQuery(""); setAlertsOnly(false); const input = document.getElementById("archivo-manifiesto") as HTMLInputElement | null; if (input) input.value = ""; };
  const houses = useMemo(() => {
    const q = query.trim().toLowerCase();
    return (preview?.casas ?? []).filter((h) => {
      const text = [h.numeroHouse ?? h.house, h.naturalezaCantidad, h.remitenteNombre, h.destinatarioNombre, h.destinatarioCarnet, h.telefonoDestinatario, h.direccionDestinatario, h.unidadDestino].filter(Boolean).join(" ").toLowerCase();
  ...
  return <main className="min-h-screen bg-[#f4f7fb] pb-16 text-slate-900"><div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8">
    <header className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><a href="/dashboard" className="text-sm font-medium text-slate-500 hover:text-slate-900">← Dashboard</a><div className="mt-3 flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-950 text-xl text-white">⇧</div><div><h1 className="text-3xl font-black tracking-tight">Centro de Control de Importación</h1><p className="mt-1 text-sm text-slate-500">Validación, ejecución y trazabilidad en tiempo real.</p></div></div></div><div className="flex items-center gap-2 text-xs font-bold text-emerald-700"><span className="h-2 w-2 rounded-full bg-emerald-500"/> API operativa</div></header>
    ...
    {loading && <Banner tone="blue" title="Analizando manifiesto" text="Leyendo estructura, Houses, personas y diagnóstico de direcciones…" spin/>}
    {error && <Banner tone="red" title="Operación detenida" text={error}/>} 

    {preview?.ok && preview.total && <>
      <section className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Kpi label="Master AWB" value={preview.metadata?.masterAwb ?? "—"} detail={preview.metadata?.fecha ? formatDate(preview.metadata.fecha) : "Fecha no detectada"}/><Kpi label="Houses" value={preview.total.cantidadHouses} detail={`${preview.registros ?? preview.total.cantidadHouses} filas reconocidas`}/><Kpi label="Bultos" value={preview.total.cantidadSacas} detail="Declarados"/><Kpi label="Personas" value={preview.total.cantidadPersonas} detail="Declaradas"/><Kpi label="Peso total" value={`${Number(preview.total.pesoTotalKg).toFixed(2)} kg`} detail="Declarado"/></section>

      <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-black">Diagnóstico antes de ejecutar</h2><Badge tone={preview.archivo?.duplicado ? "amber" : "green"}>{preview.archivo?.duplicado ? "DUPLICADO" : "LISTO"}</Badge></div><p className="mt-1 text-sm text-slate-500">Los datos de esta sección son lectura real del API; aquí todavía no se modifica la base de datos.</p></div><div className="flex flex-wrap gap-2"><Info label="Agente" value={preview.metadata?.agenteTransitario ?? "—"}/><Info label="Origen" value={preview.metadata?.paisOrigen ?? "—"}/><Info label="Consignatario" value={preview.metadata?.consignatario ?? "—"}/></div></div><div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-5"><Stage title="Archivo" text="Recibido" done/><Stage title="Parser" text="Estructura válida" done/><Stage title="Personas" text={`${preview.total.cantidadPersonas} declaradas`} done/><Stage title="Direcciones" text={`${preview.direcciones?.total ?? 0} diagnosticadas`} done={!!preview.direcciones}/><Stage title="Geocodificación" text="Se ejecutará al importar" done={false}/></div></section>

      {preview.direcciones && <section className="mt-6 rounded-3xl bg-slate-950 p-6 text-white shadow-lg"><div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-emerald-400"/><h2 className="text-xl font-black">Inteligencia de direcciones</h2></div><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">Detectamos reutilización, direcciones nuevas y casos que requerirán geocodificación o revisión.</p><div className="mt-5 h-2.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-emerald-400" style={{width:`${preview.direcciones.cobertura}%`}}/></div></div><div className="grid w-full gap-2 sm:grid-cols-2 xl:w-[650px] xl:grid-cols-4"><Dark label="Cobertura" value={`${preview.direcciones.cobertura}%`}/><Dark label="Reutilizables" value={preview.direcciones.reutilizables}/><Dark label="Nuevas" value={preview.direcciones.nuevas}/><Dark label="Pendientes" value={preview.direcciones.pendientesGeocodificacion}/></div></div></section>}

      <section className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-lg font-black">Houses detectados</h2><p className="mt-1 text-sm text-slate-500">{houses.length} de {(preview.casas ?? []).length} visibles.</p></div><div className="flex flex-col gap-2 sm:flex-row"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar House, persona, carnet, dirección…" className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm outline-none sm:w-80"/><button type="button" onClick={()=>setAlertsOnly(v=>!v)} className={`rounded-xl border px-4 py-2.5 text-sm font-bold ${alertsOnly ? "border-amber-300 bg-amber-50 text-amber-800" : "border-slate-300"}`}>{alertsOnly ? "Solo alertas ✓" : "Solo alertas"}</button></div></div><div className="max-h-[560px] overflow-auto"><table className="min-w-[1200px] w-full text-left text-sm"><thead className="sticky top-0 z-10 bg-slate-100 text-[11px] uppercase tracking-wider text-slate-500"><tr>{["House","Naturaleza","Peso","Bultos","Remitente","Destinatario","Identificación","Dirección","Estado","Destino"].map(h=><th key={h} className="px-4 py-3">{h}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{houses.map((h,i)=>{const incomplete=!h.destinatarioNombre||!h.direccionDestinatario; return <tr key={`${h.numeroHouse ?? h.house}-${i}`} className="align-top hover:bg-slate-50"><td className="whitespace-nowrap px-4 py-3 font-black">{h.numeroHouse ?? h.house ?? "—"}</td><td className="px-4 py-3 text-slate-600">{h.naturalezaCantidad ?? "—"}</td><td className="whitespace-nowrap px-4 py-3">{Number(h.pesoKg ?? 0).toFixed(2)} kg</td><td className="px-4 py-3">{Number(h.bultos ?? h.cantidadBultos ?? 0)}</td><td className="min-w-[180px] px-4 py-3">{h.remitenteNombre ?? "—"}</td><td className="min-w-[180px] px-4 py-3 font-bold">{h.destinatarioNombre ?? "—"}</td><td className="px-4 py-3">{h.destinatarioCarnet ?? "—"}</td><td className="min-w-[260px] px-4 py-3 text-slate-600">{h.direccionDestinatario ?? "—"}</td><td className="px-4 py-3"><Badge tone={incomplete ? "amber" : "green"}>{incomplete ? "REVISAR" : "DATOS OK"}</Badge></td><td className="px-4 py-3">{h.unidadDestino ?? "—"}</td></tr>})}</tbody></table></div></section>
      ...
      {!p && <section className="mt-6 rounded-3xl bg-slate-950 p-6 text-white"><div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-xl font-black">Ejecutar importación</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">La ejecución abrirá un trabajo monitorizado. Verás Houses, personas y direcciones procesadas, dirección actual, velocidad, ETA, cobertura y errores reales.</p></div><button type="button" disabled={starting || !!preview.archivo?.duplicado} onClick={startImport} className="min-w-[230px] rounded-2xl bg-white px-6 py-3.5 text-sm font-black text-slate-950 disabled:opacity-50">{starting ? "Iniciando…" : preview.archivo?.duplicado ? "Manifiesto duplicado" : "Importar y monitorizar"}</button></div></section>}
    </>}

    {p && <LiveImport progress={p} housePct={housePct} addressPct={addressPct} resolved={resolved} onReset={clear}/>} 
  </div></main>;
}

function LiveImport({progress:p,housePct,addressPct,resolved,onReset}:{progress:Progress;housePct:number;addressPct:number;resolved:number;onReset:()=>void}) {
  const running=p.status==="running"; const title=p.stage==="completed"?"Operación completada":p.stage==="failed"?"Operación con errores":"Importación en curso";
  return <section className="mt-6 space-y-5"><div className={`rounded-3xl border p-6 shadow-sm ${p.status==="completed" ? "border-emerald-200 bg-emerald-50" : p.status==="failed" ? "border-red-200 bg-red-50" : "border-blue-200 bg-white"}`}><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex items-center gap-3"><span className={`h-3 w-3 rounded-full ${running ? "animate-pulse bg-blue-500" : p.status==="completed" ? "bg-emerald-500" : "bg-red-500"}`}/><h2 className="text-2xl font-black">{title}</h2></div><p className="mt-2 text-sm text-slate-600">{p.message}</p></div><Badge tone={p.status==="completed"?"green":p.status==="failed"?"red":"blue"}>{p.stage.replace("_"," ").toUpperCase()}</Badge></div><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><LiveKpi label="Houses" value={`${p.processedHouses} / ${p.totalHouses}`} sub={`${housePct}%`}/><LiveKpi label="Personas" value={`${p.processedPeople} / ${p.totalPeople}`} sub="únicas verificadas"/><LiveKpi label="Direcciones" value={`${p.processedAddresses} / ${p.totalAddresses}`} sub={`${addressPct}% procesadas`}/><LiveKpi label="Cobertura" value={`${p.coverage}%`} sub={`${resolved} con resultado`}/></div><div className="mt-5 grid gap-4 lg:grid-cols-2"><ProgressBar label="Procesamiento de Houses" pct={housePct}/><ProgressBar label="Procesamiento de direcciones" pct={addressPct}/></div></div>
    <div className="grid gap-5 lg:grid-cols-[1.4fr_.6fr]"><section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><h3 className="text-lg font-black">Telemetría de ejecución</h3><div className="mt-5 grid gap-3 sm:grid-cols-2"><LiveKpi label="Velocidad" value={`${p.housesPerMinute.toFixed(1)} Houses/min`} sub="ritmo observado"/><LiveKpi label="Tiempo transcurrido" value={formatDuration(p.elapsedMs)} sub="desde el inicio"/><LiveKpi label="ETA" value={p.etaSeconds == null ? "Calculando…" : formatDuration(p.etaSeconds*1000)} sub="estimación basada en ritmo real"/><LiveKpi label="Errores / revisión" value={`${p.errors} / ${p.addressesReview}`} sub={`${p.addressesNotFound} no encontradas`}/></div>{p.currentAddress && <div className="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-widest text-blue-600">Dirección actualmente procesada</div><div className="mt-2 break-words text-sm font-bold text-blue-950">{p.currentAddress}</div></div>}</section><section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><h3 className="text-lg font-black">Resultado de direcciones</h3><div className="mt-5 grid grid-cols-2 gap-3"><LiveKpi label="Geocodificadas" value={p.addressesGeocoded} sub="nuevos resultados"/><LiveKpi label="Reutilizadas" value={p.addressesReused} sub="sin nueva consulta"/><LiveKpi label="No encontradas" value={p.addressesNotFound} sub="requieren atención"/><LiveKpi label="Revisión" value={p.addressesReview} sub="casos pendientes"/></div></section></div>{p.error && <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-900">{p.error}</div>}{!running && <div className="flex justify-end"><button type="button" onClick={onReset} className="rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white">Importar otro manifiesto</button></div>}</section>;
}
