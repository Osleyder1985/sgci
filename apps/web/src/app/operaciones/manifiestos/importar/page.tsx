...
interface GeocodingDetail {
  estado: "NO_ENCONTRADA" | "REQUIERE_REVISION";
  nombre: string;
  direccion: string;
  motivo: string;
}

interface GeocodingDetailsResponse {
  ok: boolean;
  jobId: string;
  total: number;
  noEncontradas: number;
  revision: number;
  detalles: GeocodingDetail[];
}

const apiUrl = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001";

type DetailFilter = "ALL" | "NO_ENCONTRADA" | "REQUIERE_REVISION";
...
export default function ImportarManifiestoPage() {
  ...
  const startImport = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/guias/importar/job`, {
        method: "POST",
        body,
      });
      ...
      if (!response.ok || !data?.ok || !data.jobId) {
        throw new Error(
          data?.message ??
            `No fue posible iniciar la importación (HTTP ${response.status}).`,
        );
      }
  ...
  const loadDetails = async (filter: DetailFilter) => {
  ...
  const filteredDetails =
    details?.detalles.filter(
      (item) => detailFilter === "ALL" || item.estado === detailFilter,
    ) ?? [];
      ...
      <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <label
            ...
            className="block cursor-pointer rounded-2xl border-2 border-dashed border-slate-300 p-7 hover:bg-slate-50"
          >
            <div className="text-xs font-black uppercase tracking-widest text-slate-400">
              Entrada operacional
            </div>
            <div className="mt-2 text-xl font-black">
              {archivo?.name ?? "Seleccionar manifiesto"}
            </div>
            <p className="mt-1 text-sm text-slate-500">
              XLSX, XLS o CSV. La validación comienza automáticamente.
            </p>
          </label>
          {archivo && !p && (
        ...
        {loading && <Banner text="Analizando manifiesto…" />}
        {error && <Banner text={error} red />}
            ...
            <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <input
                  ...
                  placeholder="Buscar persona, carnet o dirección…"
          ...
          <section className="mt-6 space-y-5">
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <Stage
                  ...
                  text={
                    geocodingDone
                      ? "Completada"
                      : p.stage === "processing_addresses"
                        ? `${addressPct}% procesada`
                        : "Pendiente"
                  }
                />
              </div>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <Kpi
                  label="Cobertura"
                  value={`${p.coverage}%`}
                  detail={`${p.addressesGeocoded + p.addressesReused} con resultado`}
                />
              </div>

              {p.currentAddress && (
                <div className="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm font-bold text-blue-950">
                  Dirección actualmente procesada: {p.currentAddress}
                </div>
              )}
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <Kpi label="Geocodificadas" value={p.addressesGeocoded} />
                <Kpi label="Reutilizadas" value={p.addressesReused} />
                <button
                  type="button"
                  onClick={() => void loadDetails("NO_ENCONTRADA")}
                  className="rounded-2xl bg-slate-50 p-4 text-left hover:bg-amber-50"
                >
                  <div className="text-xs font-black uppercase tracking-widest text-slate-400">
                    No encontradas
                  </div>
                ...
                <button
                  type="button"
                  onClick={() => void loadDetails("REQUIERE_REVISION")}
                  className="rounded-2xl bg-slate-50 p-4 text-left hover:bg-red-50"
                >
                  <div className="text-xs font-black uppercase tracking-widest text-slate-400">
                    Revisión
                  </div>
                  ...
                  <div className="text-xs text-slate-500">Ver el caso</div>
                </button>
              </div>
            </section>

            {loadingDetails && (
              <Banner text="Cargando detalles de geocodificación…" />
            )}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <h3 className="text-lg font-black">
                      Detalle de geocodificación
                    </h3>
                    <p className="mt-1 text-sm text-slate-500">
                      Casos persistidos y asociados al job.
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => setDetailFilter("NO_ENCONTRADA")}
                      className="rounded-xl border px-3 py-2 text-xs font-bold"
                    >
                      No encontradas ({details.noEncontradas})
                    </button>
                    <button
                      type="button"
                      onClick={() => setDetailFilter("REQUIERE_REVISION")}
                      className="rounded-xl border px-3 py-2 text-xs font-bold"
                    >
                      Revisión ({details.revision})
                    </button>
                  </div>
                </div>
                <div className="mt-5 space-y-3">
                  {filteredDetails.length === 0 ? (
                    ...
                    filteredDetails.map((item, index) => (
                      <article
                        key={`${item.estado}-${item.direccion}-${index}`}
                        className="rounded-2xl border p-4"
                      >
                        <div className="flex flex-wrap items-center gap-2">
                          <span
                            className={`rounded-full px-2.5 py-1 text-[11px] font-black ${
                              item.estado === "NO_ENCONTRADA"
            ...
            {p.error && <Banner text={p.error} red />}
            {p.status !== "running" && (
...
function Banner({ text, red = false }: { text: string; red?: boolean }) {
  return (
    <div
