...
export class ManifiestosService {
  ...
  async importar(buffer: Buffer, originalname: string, jobId?: string) {
    try {
      if (jobId) await this.progress?.update(jobId, { stage: 'parsing', message: 'Analizando y validando el manifiesto.' });
      const parsed = this.parser.parse(buffer, originalname);
      this.validateParsedManifest(parsed);
      if (jobId)
  ...
  private async resolvePersona(tx: Prisma.TransactionClient, nombre: string, carnet: string | null): Promise<string> {
    ...
    if (personaExistente) {
      if (numero)
        await tx.documentoIdentidad.createMany({ data: [{ personaId: personaExistente.id, tipo: 'CARNET_IDENTIDAD', numero, numeroOriginal: carnet, esPrincipal: true }], skipDuplicates: true });
    ...
    if (numero)
      await tx.documentoIdentidad.create({ data: { personaId: persona.id, tipo: 'CARNET_IDENTIDAD', numero, numeroOriginal: carnet, esPrincipal: true } });
    return persona.id;
  }
  ...
  private async verificarDirecciones(destinatarios: DestinatarioImportado[], paisOrigen: string | null | undefined, jobId?: string): Promise<ResultadoDirecciones> {
    ...
    if (jobId) await this.progress?.update(jobId, { totalAddresses: direcciones.length, processedAddresses: 0, message: `Preparando ${direcciones.length} direcciones.` });

    for (const [index, destinatario] of direcciones.entries()) {
      const direccion = destinatario.direccion!.trim();
      if (jobId) await this.progress?.update(jobId, { processedAddresses: index, currentAddress: direccion, message: `Procesando dirección ${index + 1} de ${direcciones.length}.` });
      try {
        ...
        if (existente?.estadoGeocodificacion === 'GEOCODIFICADA') {
          ...
          if (jobId) await this.progress?.update(jobId, { processedAddresses: index + 1, addressesReused: resultado.direccionesReutilizadas, currentAddress: direccion });
          continue;
        }
        ...
        if (jobId) await this.progress?.update(jobId, { processedAddresses: index + 1, addressesGeocoded: resultado.direccionesGeocodificadas, currentAddress: direccion });
      } catch (error) {
        resultado.direccionesPendientes++;
  ...
  private cleanText(value: unknown): string | null { if (value === null || value === undefined) return null; const text = String(value).trim(); return text || null; }
  private normalizeIdentity(value: unknown): string { return String(value ?? '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase().replace(/[^A-Z0-9]/g, ''); }
  private identityKey(nombre: string, carnet: string | null): string { return `${this.normalizeIdentity(nombre)}|${this.normalizeIdentity(carnet)}`; }
  private handleError(error: unknown): never { if (error instanceof BadRequestException) throw error; if (error instanceof Error) throw new BadRequestException(error.message); throw new BadRequestException(String(error)); }
}
