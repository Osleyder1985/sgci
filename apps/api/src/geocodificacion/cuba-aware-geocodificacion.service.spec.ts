...
describe('CubaAwareGeocodificacionService', () => {
  ...
  it(
    'quita el país final CUBA antes de delegar para conservar municipio y provincia',
    async () => {
  ...
  it(
    'no intenta geocodificar una dirección cubana que solo contiene calle y país',
    async () => {
      process.env.LOCATIONIQ_API_KEY = 'test-key';
  ...
  it(
    'no agrega CUBA dos veces cuando el catálogo territorial ya resolvió la provincia',
    async () => {
      process.env.LOCATIONIQ_API_KEY = 'test-key';
