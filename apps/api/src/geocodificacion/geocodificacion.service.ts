...
export class GeocodificacionService {
  ...
  private readonly baseUrl = process.env.GEOCODER_BASE_URL?.trim() || 'https://us1.locationiq.com/v1/search';
  ...
  async geocodificar(direccion: string, pais?: string | null): Promise<GeocodingResult | null> {
    if (!this.apiKey) throw new Error('LOCATIONIQ_API_KEY no está configurada. No se puede geocodificar una dirección nueva.');
    ...
    this.logger.log(`Geocodificando: "${texto}" -> "${normalizada.canonica}"${esCuba ? ' | país=CUBA' : paisBusqueda ? ` | país=${paisBusqueda}` : ''}`);
  ...
  private async buscarLocationIq(query: string, textoFallback: string): Promise<GeocodingResult | null> {
    ...
    try {
      ...
      if (!Number.isFinite(lat) || !Number.isFinite(lon)) throw new Error('LocationIQ devolvió coordenadas inválidas.');
      ...
      this.logger.warn(`No fue posible geocodificar "${query}": ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }

  private normalizarDireccionCubana(direccion: string): DireccionCubanaNormalizada {
    const limpia = direccion.toUpperCase().replace(/[\r\n]+/g, ' ').replace(/\s+/g, ' ').trim();
    const sinZona = limpia.replace(/\s*\(\s*ZONA\s*\d+\s*\)\s*$/i, '').replace(/\s+ZONA\s*\d+\s*$/i, '').trim();
    const partes = sinZona.split(',').map((parte) => parte.trim().replace(/\s+/g, ' ')).filter(Boolean);
    ...
    for (const componente of principales.slice(1)) this.extraerComponente(componente, resultado);
  ...
  private extraerMarcadores(texto: string, resultado: DireccionCubanaNormalizada): void {
    ...
    if (edificio) {
      ...
      restante = `${restante.slice(0, edificio.index)} ${restante.slice((edificio.index ?? 0) + edificio[0].length)}`.trim();
    }

    const apartamento = restante.match(/\b(?:APARTAMENTO|APTO\.?)\s*#?\s*([A-Z0-9-]+)/i);
    if (apartamento) {
      resultado.apartamento = apartamento[1];
      restante = `${restante.slice(0, apartamento.index)} ${restante.slice((apartamento.index ?? 0) + apartamento[0].length)}`.trim();
    }

    const numero = restante.match(/#\s*([A-Z0-9-]+)/i);
    if (numero) {
      resultado.numeroCasa = numero[1];
      restante = `${restante.slice(0, numero.index)} ${restante.slice((numero.index ?? 0) + numero[0].length)}`.trim();
    }
  ...
  private extraerComponente(componente: string, resultado: DireccionCubanaNormalizada): void {
    ...
    if (apartamento) { resultado.apartamento ??= apartamento[1]; return; }
    const edificio = texto.match(/^\bEDIF(?:ICIO)?\.?\s*#?\s*([A-Z0-9-]+)\s*$/i);
    if (edificio) { resultado.edificio ??= edificio[1]; return; }
    const reparto = texto.match(/^\bRPTO\.?\s+(.+)$/i);
    if (reparto) { resultado.reparto ??= reparto[1].trim(); return; }
    const entre = texto.match(/^\bE\s*\/\s*(.+)$/i);
    if (entre) { resultado.entreCalles ??= this.normalizarEntrecalles(entre[1]); return; }
    const numero = texto.match(/^#\s*([A-Z0-9-]+)$/i);
    if (numero) { resultado.numeroCasa ??= numero[1]; return; }
    if (!resultado.entreCalles && /\b.+\s+Y\s+.+\b/i.test(texto)) resultado.entreCalles = this.normalizarEntrecalles(texto);
  }

  private normalizarEntrecalles(texto: string): string {
    return texto.replace(/^\s*(?:E\s*\/|ENTRE)\s+/i, '').replace(/\s+/g, ' ').trim();
  }
  ...
  private limpiarProvincia(provincia: string): string {
    return provincia.replace(/\s*\(\s*ZONA\s*\d+\s*\)\s*$/i, '').replace(/\s+ZONA\s*\d+\s*$/i, '').trim();
  }
  ...
  private construirConsultas(direccion: DireccionCubanaNormalizada, pais?: string): string[] {
    ...
    const simplificada = [direccion.calle, direccion.numeroCasa, direccion.reparto ? `REPARTO ${direccion.reparto}` : undefined, direccion.municipio, direccion.provincia].filter(Boolean).join(', ');
    return [...new Set([`${completa}${sufijoPais}`.trim(), simplificada && simplificada !== completa ? `${simplificada}${sufijoPais}`.trim() : ''].filter(Boolean))];
  }

  private esDireccionCubana(original: string, normalizada: DireccionCubanaNormalizada): boolean {
    const texto = `${original} ${normalizada.municipio ?? ''} ${normalizada.provincia ?? ''}`.toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
